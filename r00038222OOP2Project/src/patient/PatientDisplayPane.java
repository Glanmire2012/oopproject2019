...
public class PatientDisplayPane extends MyGridPane {
	...
	public void buildResultFrame() {
		...
		GridPane.setConstraints(fNameText, 1, 1);

		Text sNameLabel = new Text("Last Name: ");
		GridPane.setConstraints(sNameLabel, 2, 1);
		Text sNameText = new Text(sName);
		GridPane.setConstraints(sNameText, 3, 1);

		Text Add1Label = new Text("Address: ");
		GridPane.setConstraints(Add1Label, 0, 2);
		Text Add1Text = new Text(Add1);
		GridPane.setConstraints(Add1Text, 1, 2);
		Text Add2Text = new Text(Add2);
		GridPane.setConstraints(Add2Text, 2, 2);
		Text Add3Text = new Text(Add3);
		GridPane.setConstraints(Add3Text, 3, 2);
		Text cityText = new Text(city);
		GridPane.setConstraints(cityText, 4, 2);
		Text countyText = new Text(county);
		GridPane.setConstraints(countyText, 5, 2);

		Text phoneLabel = new Text("Phone: ");
		GridPane.setConstraints(phoneLabel, 1, 3);
		Text phoneText = new Text(phone);
		GridPane.setConstraints(phoneText, 2, 3);

		enter = new Button("Open Record");
		GridPane.setConstraints(enter, 0, 3);
		enter.setOnAction(e -> openRecord());
		this.prefWidthProperty().bind(this.widthProperty());
		this.getChildren().addAll(fNameLabel, fNameText, sNameLabel, sNameText, Add1Label, Add1Text, Add2Text, Add3Text,
				cityText, countyText, phoneLabel, phoneText, IDLabel, IDText, enter);
	}
