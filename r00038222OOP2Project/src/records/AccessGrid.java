...
public class AccessGrid extends MyGridPane {
	...
	public void displayInformation() {
		...
		Button save = new Button("Save");this.setConstraints(save,1,0);save.setOnAction(e -> {appointment.setAppointmentNotes(notes.getText());instance.updatePatientList(patientList);} );
		Text proceduresPerformed = new Text("Procedures Performed : ");this.setConstraints(proceduresPerformed, 0, 7);
		ShowAddedProcedures doneGrid = new ShowAddedProcedures(proceduresDone,appointment);this.setConstraints(doneGrid, 0,8);
		
		appointment.setTotalPrice(doneGrid.getTotalPrice());
		
		Text totalPrice = new Text("Total Cost : €"+appointment.getTotalPrice());this.setConstraints(totalPrice, 0,9);
		Button genInvoice = new Button("Generate Invoice");this.setConstraints(genInvoice, 1,9);genInvoice.setOnAction(e ->{ openInvoice();});
		if (appointment.isPaid()==false) {
			Button pay = new Button("PAY");this.setConstraints(pay,2,9);pay.setOnAction(e -> {appointment.setPaid(true);instance.updatePatientList(patientList);});
			this.getChildren().add(pay);
		}else {
			Text paid = new Text("PAID");this.setConstraints(paid,2,9);
			this.getChildren().add(paid);
		}
		
		constructProcedureSelect();
		this.setConstraints(flow,0,10);
		this.getChildren().addAll(nameLabel,PIDLabel,addressLabel,phoneLabel,notesLabel,notes,flow, dateof,proceduresPerformed,doneGrid,save,totalPrice,genInvoice
				);
